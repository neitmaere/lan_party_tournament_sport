      ...
      for (var i = 1; i <= 8; i++) {
        ...
        for (var x = 0; x < 2; x++) {
          quarterFinalCell.addContent(new sap.m.StandardTile({
            title : "{SportTournament>/"+this.roundCounter+"/nickname}",
            icon : "sap-icon://customer-and-supplier",
            info : "{SportTournament>/"+this.roundCounter+"/result}",
            infoState : "{= ${SportTournament>/"+this.roundCounter+"/result} === 'WIN' ? 'Success' : 'Error'}",
            number : "{SportTournament>/"+this.roundCounter+"/gamerId}"
          }));
          this.roundCounter++;
        }
